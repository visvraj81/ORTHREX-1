            ...
            <div class="hero-container">
                <div class="banner-page__container">
                    <div class="banner-page__title">
                        ...
                        <p>{{ t($category->banner_subtitle ?? 'Advanced Solutions for Lower Limb Reconstruction.') }}</p>
                    </div>
                    <div class="banner-page__intro">
                        <p>{{ t($category->banner_description ?? 'Explore our comprehensive range of Lower Limb implants, precision-engineered to deliver strength, stability, and reliable performance for advanced orthopaedic procedures.') }}</p>
                        <nav class="breadcrumb">
                            <a href="{{ route('home') }}" class="breadcrumb-link">{{ t('Home') }}</a>
                            <i class="breadcrumb-divider fa-solid fa-chevron-right"></i>
                            <a href="#" class="breadcrumb-link">{{ t($category->title) }}</a>
                        </nav>
                    </div>
                </div>
            </div>
            ...
            <div class="hero-container">
                ...
                <div class="work-process__container">
                    <div class="case-studies__heading">
                        <div class="sub-heading-container text-center">{{ t($category->section_title) }}</div>
                        <div class="sub-heading-divider-about"></div>
                        <p class="text-center">{{ t($category->section_description) }}</p>
                    </div>
                </div>
				@endif
                        ...
                        @foreach($products as $product)
                            <div class="product-card-inner">
                                ...
                                <h3 style="font-family: 'Inter', sans-serif; font-size: 20px; font-weight: 700; color: #1c1c1c; text-transform: uppercase; margin: 0 0 15px 0; line-height: 1.3; letter-spacing: 0.3px;">
                                    <a href="{{ route('products-details', ['id' => $product->id]) }}" style="color: #1c1c1c; text-decoration: none; display: block;">{{ t($product->title) }}</a>
                                </h3>
                                <a href="{{ route('products-details', ['id' => $product->id]) }}">{{ t('View Details') }}</a>
                            </div>
                        @endforeach
                    @else
                        ...
                        <div style="grid-column: 1 / -1; display: flex; justify-content: center; align-items: center; width: 100%; padding: 60px 20px;">
                            <div style="font-family: sans-serif; text-align: center; max-width: 500px; background: transparent;">
                                ...
                                <!-- Perfect Typography -->
                                <h2 style="font-size: 26px; color: #111; margin-bottom: 12px; font-weight: 700;">{{ t('No Product Found') }}</h2>
                                <p style="font-size: 15px; color: #666; margin: 0 auto; line-height: 1.6;">
                                    {{ t("We couldn't find any orthopedic products at the moment.") }}<br>
                                    {{ t('Please check back later or explore other categories.') }}
                                </p>
                            </div>
                        </div>
                    @endif
